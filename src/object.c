...
int object_read(struct volume_object object, uint32_t offt, char *buf, uint32_t len, uint32_t *olen) {
	...
	if ((p->flag & OBJECT_CACHE_NOT_PRESENT) && !trxlog_match(&p->trxlog, offt, len)) {
...
int object_write(struct volume_object object, uint32_t offt, const char *buf, uint32_t len) {
	struct object_cache *p;
	int ret;
...
static inline bool object_equals(struct volume_object a, struct volume_object b) {
	return (a.index == b.index && a.chunk == b.chunk);
}

struct object_cache *object_cache_create_and_aquire(struct volume_object object) {
	struct object_cache *p;
...
struct object_cache *object_cache_lookup_and_acquire(struct volume_object object) {
	struct object_cache *p;
...
int object_cache_fulfill(struct object_cache *p) {
	...
	while (from < to) {
		...
		if (!mark && (ret = object_cache_intr_ptr->write(p, from, sbuf, len)) != SUCCESS) {
			free(rbuf);
...
int object_cache_flush(struct object_cache *p) {
	...
	while (len < OBJECT_MAX_SIZE) {
		...
		if ((ret = object_cache_intr_ptr->read(p, len, rbuf + len, &rlen)) != SUCCESS) {
			free(buf);
			return ret;
		}
		if (!rlen)
	...
	if (memcmp(new_md5, p->md5, OBJECT_MD5_DIGEST_LENGTH) != 0) {
		...
		if ((ret = volume_put_object(p->object, buf, OBJECT_MD5_DIGEST_LENGTH + len)) != SUCCESS) {
			free(buf);
			return ret;
		}
	}
