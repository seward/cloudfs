...
struct vfs_inode **vfs_dir_read(uint64_t inode, struct vfs_inode_ptr *empty_ptr) {
	...
	if (empty_ptr && !found_empty)
		vfs_dir_read_pass(&ptr, empty_ptr, 
				NULL, sizeof(node->data));
	return node_list;
}
...
bool vfs_dir_read_pass(struct vfs_inode_ptr *ptr, struct vfs_inode_ptr *save_ptr, 
	...
	if ((ret = object_read(ptr->object, ptr->offt, data, len, &rlen)) != SUCCESS) {
		if (ret != NOT_FOUND)
...
int vfs_node_lookup(const char *path, struct vfs_inode **out_node, bool new_file) {
...
int vfs_node_rehash(struct vfs_inode *node) {
	...
	if (object_read(node->ptr.object, node->ptr.offt, (char*) &node_data, 
			sizeof(node_data), &rlen) != SUCCESS || rlen < sizeof(node_data)) {
		warning("Read error on inode %016" PRIx64, node->ptr.object.index);
		return -EFAULT;
	}
	if (node->data.ino == node_data.ino && (node_data.flag & VFS_INODE_FLAG_VALID))
...
static inline mode_t normalize_mode(mode_t mode, mode_t default_mt, bool force) {
	mode_t mt;
...
int vfs_fuse_symlink(const char *from, const char *to) {
	...
	if ((ret = vfs_io_perform(node, VFS_IO_WRITE, (char*) from, strlen(from), 0)) != 0) {
		vfs_node_free(node);
		return ret;
	}
	return vfs_node_commit_and_free(node);
...
int vfs_fuse_write(const char *path, const char *buf, uint64_t size,
	...
	node->modified        = true;
	if ((ret = vfs_io_perform(node, VFS_IO_WRITE, (char*) buf, size, offset)) != 0)
		return ret;
	return size;
}
